...
2. Every error has a name. Don't say "handle errors." Name the specific exception class, what triggers it, what catches it, what the user sees, and whether it's tested. catch(e) without type-narrowing is a code smell — call it out.
...
* ASCII diagrams in code comments for complex designs — Components (state transitions), Services (pipelines), Pages/Routes (request flow), Hooks (composable behavior), Tests (non-obvious setup).
...
Step 0 > System audit > Error/catch map > Test diagram > Failure modes > Opinionated recommendations > Everything else.
Never skip Step 0, the system audit, the error/catch map, or the failure modes section. These are the highest-leverage outputs.
...
```
  ...
  -------------------------|-----------------------------|-----------------
  exampleService.call()    | API timeout                 | AbortError
                           | API returns 429             | RateLimitError
                           | API returns malformed JSON  | SyntaxError
                           | GraphQL client error        | GraphQL client errors
  ...
  TypeError                    | N ← GAP   | —                      | 500 error ← BAD
  NotFoundError                | Y         | Return null, log warning | "Not found" message
```
Rules for this section:
* `catch (e)` without type-narrowing is ALWAYS a smell. Narrow the error type.
...
Evaluate:
* N+1 queries. For every new GraphQL query or data fetching hook: is there over-fetching or waterfall risk?
...
### Error & Catch Registry (from Section 2)
Complete table of every method that can fail, every exception class, caught status, catch action, user impact.
...
```
  ...
  | Error/catch registry | ___ methods, ___ CRITICAL GAPS              |
