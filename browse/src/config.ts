 ...
 *   2. git rev-parse --show-toplevel → projectDir/.local-context/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .local-context/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.local-context');
...
/**
 * Create the .local-context/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .local-context/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.local-context\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.local-context/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
