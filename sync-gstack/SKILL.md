...
### Step 2 — Create sync branch from origin/main and merge upstream

Create a branch based on `origin/main` (the fork), then merge upstream into it. This
preserves all existing fork adaptations and only introduces the upstream delta — avoiding
unnecessary merge conflicts when the PR lands.
...
**If there are merge conflicts:** Resolve them, preferring the fork's version for:
...
**If the merge is clean:** Continue — Steps 3–4 will verify and adapt any new content.

### Step 3 — Verify generator (preamble trim)

Since the branch started from `origin/main`, the generator already has the fork's
adaptations. This step verifies the merge didn't reintroduce upstream content and
fixes it if it did.
...
If `generatePreamble()` already matches this (no update check, no session tracking, no
contributor mode), skip this step — upstream may have adopted the same trim.
...
**Verification:**
```bash
grep -n '_UPD=\|UPGRADE_AVAILABLE\|Contributor Mode\|session_id\|### Rails' scripts/gen-skill-docs.ts
```
...
This step verifies the merge didn't reintroduce upstream content (Rails vocab, Greptile,
...
|-------|----------|------------|
| gstack (root) | SKILL.md.tmpl | Preamble trim via generator; .gstack/ → .local-context/ |
